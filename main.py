...
TAG_COLORS = {
    ...
    "seminar": "tag", "study": "tag", "conference": "tag", "studio": "tag",
}
...
def get_room_bookings(room, booking_date):
    return [b for b in st.session_state.bookings
            if b["room"] == room and b["date"] == booking_date]
        ...
        with col_data:
            ...
            if st.button("Save Changes", use_container_width=True, key="pf_save"):
                if not new_first or not new_last:
                    ...
                    st.session_state.users[uid].update({
                        "first_name": new_first.strip(),
                        "last_name":  new_last.strip(),
                        "name":       f"{new_first.strip()} {new_last.strip()}",
        ...
        with col_pw:
            ...
            if st.button("Update Password", use_container_width=True, key="cp_btn"):
                ...
                if st.session_state.users[uid]["password"] != hash_password(old_pw):
                    ...
                    st.session_state.users[uid]["password"] = hash_password(new_pw1)
                    save_users(st.session_state.users)
